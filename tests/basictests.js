...
var oknok = require('../index');
var callNextTick = require('call-next-tick');

test('Basic success', function basicSuccess(t) {
  successfulCall(
    oknok({ ok: checkSuccess, nok: checkOuterCallback, log: checkLog })
  );
  ...
  function checkSuccess(result1, result2, result3, outerDone) {
    ...
    t.equal(
      outerDone,
      checkOuterCallback,
      'outerDone is passed to checkSuccess'
    );
    t.end();
  }
});
...
test('Basic failure', function basicFailure(t) {
  ...
  function checkOuterCallback(error) {
    t.equal(
      error.message,
      'Call failed.',
      'Done is called with an error when call fails.'
    );
  }
  ...
  function checkLog(error, stack) {
    t.equal(
      error.message,
      'Call failed.',
      'Log is called with an error when call fails.'
    );
